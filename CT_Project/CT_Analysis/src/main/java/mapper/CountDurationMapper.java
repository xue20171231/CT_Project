...
public class CountDurationMapper extends TableMapper<ComDimension, Text> {
    ...
    protected void map(ImmutableBytesWritable key, Result value, Context context) throws IOException, InterruptedException {
        ...
        //组装ComDimension
        //1、组装时间维度DataDimension
        //-1代表整个月或者日
        DateDimension yearDataDimension = new DateDimension(year, "-1", "-1");
        DateDimension monthDataDimension = new DateDimension(year, month, "-1");
        DateDimension dayDataDimension = new DateDimension(year, month, day);

        //2、组装联系人维度ContactDimension
        ContactDimension callerContactDimension = new ContactDimension(caller, phoneNameMap.get(caller));

        //聚合主叫数据
        comDimension.setContactDimen(callerContactDimension);
        //年时间维度
        comDimension.setDataDimen(yearDataDimension);
        context.write(comDimension,durationText);
        //月时间维度
        comDimension.setDataDimen(monthDataDimension);
        context.write(comDimension,durationText);
        //日时间维度
        comDimension.setDataDimen(dayDataDimension);
        context.write(comDimension,durationText);

        //2.1、组装联系人维度ContactDimension
        ContactDimension calleeContactDimension = new ContactDimension(callee, phoneNameMap.get(callee));

        //聚合主叫数据
        comDimension.setContactDimen(calleeContactDimension);
        //年时间维度
        comDimension.setDataDimen(yearDataDimension);
        context.write(comDimension,durationText);
        //月时间维度
